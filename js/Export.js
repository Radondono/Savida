var Export = {
    buildData: function() {
        ...
        var config = {
            ...
            characters: chars.map(function(c) {
                return {
                    ...
                    poses: c.poses || ['default'],
                    expressions: c.expressions || ['neutral'],
                    currentPose: c.currentPose || 'default',
                    currentExpression: c.currentExpression || 'neutral'
                };
            }),
        ...
        var scenes = Canvas.nodes.filter(function(n) { return n.type === 'scene'; }).map(function(n) {
            var scene = {
                ...
                choices: (n.choices || []).map(function(ch, i) {
                    ...
                    chars.forEach(function(c) {
                        ...
                        if (ch['pose_' + c.id]) obj['pose_' + c.id] = ch['pose_' + c.id];
                        if (ch['expr_' + c.id]) obj['expr_' + c.id] = ch['expr_' + c.id];
            ...
            chars.forEach(function(c) {
                if (n['pose_' + c.id]) scene['pose_' + c.id] = n['pose_' + c.id];
                if (n['expr_' + c.id]) scene['expr_' + c.id] = n['expr_' + c.id];
            });
    ...
    exportHTML: function() {
        ...
        engineHTML += '<script>\n' + jsContent + '\n' +
            ...
            'function initState(){characters.forEach(function(c){characterClothing[c.id]={};characterState[c.id]={pose:c.currentPose||"default",expr:c.currentExpression||"neutral"};(c.clothing||[]).forEach(function(cloth){characterClothing[c.id][cloth]=true;});});}\n' +
            ...
            'function updateCharImage(charId){var container=document.getElementById("img_"+charId);if(!container)return;var c=characters.find(function(x){return x.id===charId;});if(!c)return;var st=characterState[charId]||{pose:"default",expr:"neutral"};var poseKey=st.pose||"default";var exprKey=st.expr||"neutral";var cimgs=images&&images[charId];var poseData=(cimgs&&cimgs.poses&&cimgs.poses[poseKey])?cimgs.poses[poseKey]:{base:null,clothing:{},expressions:{}};container.innerHTML="";var has=false;if(poseData.base){var img=document.createElement("img");img.src=poseData.base;img.style.zIndex="1";container.appendChild(img);has=true;}var cs=characterClothing[charId]||{};(c.clothing||[]).forEach(function(cn,ci){if(cs[cn]&&poseData.clothing&&poseData.clothing[cn]){var cimg=document.createElement("img");cimg.src=poseData.clothing[cn];cimg.style.zIndex=(2+ci);container.appendChild(cimg);has=true;}});var exprData=(poseData.expressions&&poseData.expressions[exprKey])?poseData.expressions[exprKey]:{eyes:null,mouth:null};if(exprData.eyes){var eimg=document.createElement("img");eimg.src=exprData.eyes;eimg.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:contain;z-index:99;pointer-events:none;";container.appendChild(eimg);}if(exprData.mouth){var mimg=document.createElement("img");mimg.src=exprData.mouth;mimg.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:contain;z-index:100;pointer-events:none;";container.appendChild(mimg);}if(!has&&!exprData.eyes&&!exprData.mouth){container.innerHTML=\'<div class="no-image">\'+(c.icon||"🧑")+\'</div>\';}updateClothIndicator(charId);}\n' +
            ...
            'function show(id){var s=STORY_SCENES.find(function(x){return x.id===id});if(!s){if(STORY_ENDINGS[id]){document.getElementById("game").style.display="none";document.getElementById("endScreen").style.display="block";document.getElementById("endTitle").textContent=STORY_ENDINGS[id].title;document.getElementById("endDesc").textContent=STORY_ENDINGS[id].desc;return;}alert("Scene: "+id+" not found");return;}document.getElementById("txt").textContent=s.text;if(s.subtitle)document.getElementById("sub").textContent=s.subtitle;updateCharVisibility(s);characters.forEach(function(c){if(s["pose_"+c.id]&&s["pose_"+c.id]!=="")characterState[c.id].pose=s["pose_"+c.id];if(s["expr_"+c.id]&&s["expr_"+c.id]!=="")characterState[c.id].expr=s["expr_"+c.id];});updateAllImages();var ca=document.getElementById("choices-area");ca.innerHTML="";s.choices.forEach(function(ch){var b=document.createElement("button");b.textContent=ch.text;b.onclick=function(){characters.forEach(function(c){applyClothing(c.id,ch["strip_"+c.id],ch["wear_"+c.id]);if(ch["pose_"+c.id]&&ch["pose_"+c.id]!=="")characterState[c.id].pose=ch["pose_"+c.id];if(ch["expr_"+c.id]&&ch["expr_"+c.id]!=="")characterState[c.id].expr=ch["expr_"+c.id];});show(ch.next);};ca.appendChild(b);});}\n' +
            'initState();buildAllChars();if(STORY_SCENES.length>0)show(STORY_SCENES[0].id);\n' +
            '<' + '/script></body></html>';
